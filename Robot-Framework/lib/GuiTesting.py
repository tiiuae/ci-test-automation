...
def text_matches(text, substring_to_find, ignore_case=True, precision=100):
    ...
    matched =  result.score >= int(precision)
...
def locate_text(screenshot, text, scale=1, precision=100, ignore_case=True):
    logging.info("Searching for" + text)
    data = get_data_from_image(screenshot, scale)
    visible, matched_text = is_text_in_parsed_image_data(data, text, ignore_case=ignore_case, precision=precision)

    if visible:
        # Loop through results to find matching text
        for i, word in enumerate(data['text']):
            if matched_text.strip().lower() in word.strip().lower():
                x, y, w, h = (data[key][i] for key in ['left', 'top', 'width', 'height'])
                x, y, w, h = (v // scale for v in (x, y, w, h))
                text_center = (x + w // 2, y + h // 2)
                logging.info(f"Looking for '{text}' with precision {precision}%. Found '{matched_text}' at {text_center}")
