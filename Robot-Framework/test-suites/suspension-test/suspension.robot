...
#    Verify login
    ...
    Log to console    ${start_time}: waiting for 4 min
    Sleep          240
    Check screen brightness  24000
    Log to console    ${start_time}: waiting for 10 sec
    Sleep          10
    Check notification       The system will suspend soon due to inactivity.    ${last_id}
    Check the screen state   on
    Log to console    ${start_time}: waiting for 50 sec
    Sleep          50
    # somehow check that screen is locked
    Log to console    ${start_time}: waiting for 2.5 min
    Sleep          150
    Check the screen state   off
    Log to console    ${start_time}: waiting for 7.5 min
    Sleep          450
...
Check notification
    ...
    FOR    ${notification}    IN    @{notifications}
        ...
        IF    "${body_text}[0]" == "${text}"
            Append To List    ${matching_ids}    ${id}[0]
        ELSE
            Log    "${body_text}[0]" != "${text}"
        END
    END
