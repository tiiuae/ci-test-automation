...
Check that internal tcp sync probing triggers blacklisting
...
Check that external ping flooding triggers blacklisting
    [Tags]            SP-T299  SP-T299-3  lenovo-x1  darter-pro  dell-7330  lab-only
...
Check that external tcp sync probing triggers blacklisting
    [Tags]            SP-T299  SP-T299-4  lenovo-x1  darter-pro  dell-7330  lab-only
...
External Tcp Syn Flood NetVM
    ...
    ${rc}   ${first_burst}   Run And Return Rc And Output  nix-shell -p hping --command 'hping3 -S -p 22 -i u10000 -c 20 ${DEVICE_IP_ADDRESS}'
    Log     ${first_burst}
    Should Be Equal As Integers    ${rc}    0    hping command failed
    Should Contain     ${first_burst}    flags=
    ${rc}   ${second_burst}   Run And Return Rc And Output  nix-shell -p hping --command 'hping3 -S -p 22 -i u10000 -c 20 ${DEVICE_IP_ADDRESS}'
    Log     ${second_burst}
    Should Contain     ${second_burst}    0 packets received
