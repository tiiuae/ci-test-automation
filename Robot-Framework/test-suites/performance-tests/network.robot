...
...                 AND  Select The Network Connection To Use 
...
Check iperf3 got results
    [Documentation]    Check if iperf3 results were got. When something goes wrong output is '${output1}= <result object with rc 1>'
    [Arguments]        ${result1}=${EMPTY}  ${result2}=${EMPTY}
    ${failure}    Run Keyword and return status      should contain  ${result1}-${result2}    rc 1>
