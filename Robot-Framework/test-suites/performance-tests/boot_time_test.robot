...
Get Boot times
    ...
    &{statistics_desktop}  Get From Dictionary   ${statistics}   time_to_desktop
    &{statistics_ping}     Get From Dictionary   ${statistics}   response_to_ping

    ${fail_msg}=  Set Variable  ${EMPTY}
    IF  "${statistics_desktop}[flag]" == "1"
        ${add_msg}      Create deviation message  ${statistics_desktop}
        ${fail_msg}=    Set Variable  Time to desktop:\n${add_msg}
    END
    IF  "${statistics_ping}[flag]" == "1"
        ${add_msg}      Create deviation message  ${statistics_ping}
